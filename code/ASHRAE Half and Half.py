...
if __name__ == '__main__':
    path_data = "../ASHRAE - Great Energy Predictor III/"
    path_train = path_data + "train.csv"
    path_test = path_data + "test.csv"
    path_building = path_data + "building_metadata.csv"
    path_weather_train = path_data + "weather_train.csv"
    path_weather_test = path_data + "weather_test.csv"
    path_leak = "../ASHRAE Leak Data Station/leak.feather"
    ...
    df_leak = pd.read_feather(path_leak)

    df_train = pd.read_csv(path_train)
    df_train.timestamp = pd.to_datetime(df_train.timestamp, format="%Y-%m-%d %H:%M:%S")

    print(df_leak.dtypes)
    print(df_train.dtypes)


    # df_train = pd.merge(df_train, df_leak, how='right', on=['building_id', 'meter', 'meter_reading', 'timestamp'])
    df_train = pd.concat([df_train, df_leak], axis=0)
    ...
    submission.to_csv("submission_train_with_leak.csv", index=False)
