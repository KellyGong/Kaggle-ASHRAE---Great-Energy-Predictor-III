...
if __name__ == '__main__':
    DATA_PATH = "../ASHRAE - Great Energy Predictor III/"
    leak_df_path = "../ASHRAE Leak Data Station/leak.feather"
    train_df = pd.read_csv(DATA_PATH + 'train.csv')
    leak_df = pd.read_feather(leak_df_path)
    # Remove outliers
    train_df = train_df[train_df['building_id'] != 1099]
    train_df = train_df.query('not (building_id <= 104 & meter == 0 & timestamp <= "2016-05-20")')
    train_df.timestamp = pd.to_datetime(train_df.timestamp, format="%Y-%m-%d %H:%M:%S")
    train_df = pd.concat([train_df, leak_df], axis=0)
    ...
    del row_ids, results
    gc.collect()
    results_df.to_csv("submission_train_with_leak.csv", index=False)